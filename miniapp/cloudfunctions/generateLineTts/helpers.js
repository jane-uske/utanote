...
function envNumber(name, fallback, min, max) {
  ...
  const lo = min == null ? safe : Math.max(min, safe)
...
function stableHash(value, len = 32) {
...
function buildAssetKey({ source, songId, cardId, lineId, assetType, chunkKey, voice, speaker, speedScale, audioText }) {
  const value = [
    ...
    assetType || '',
    chunkKey || '',
    normalizeText(audioText || ''),
    voice || DEFAULT_VOICE,
    voiceToSpeaker(voice, speaker),
    normalizeSpeedScale(speedScale),
  ].join('\n')
