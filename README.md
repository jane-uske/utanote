...
- 🎤 **唱法讲解** — 每句歌词一键查看歌唱发音要点（音拍切分/长音/促音/助词读法），固定问题、全局缓存，同一句全网只生成一次
...
│   │   │   ├── ai.js          askLine 云函数调用（唱法讲解）+ 本地答案缓存
...
│   │   ├── askLine/           逐句唱法讲解云函数（DeepSeek + 内容寻址全局缓存 + 额度）
│   │   ├── initTtsCollections/ 幂等建齐数据库集合（bootstrap）
│   │   └── login/             微信静默登录云函数
│   └── package.json
│
├── scripts/
